...
def parse_input(argv):
    ...
    args = [float(coord.strip()) for coord in argv]
    #print(args)
    point = Point(args[1], args[0]) # Shapely requires a lon, lat point
...
def run_query_arbitrary(inputs):

    
    # Gets information from the user input
    args = parse_input(inputs)
    #print(args)
...
def main(argv):
    ...
    if len(argv) != 3:
        print("Usage: python find_radius.py lat lon deaths")
        sys.exit(1)
    
    # Gets input from user and turns it into a shapely point and an integer representing an arbitrary number of deaths
    return run_query_arbitrary(argv)
